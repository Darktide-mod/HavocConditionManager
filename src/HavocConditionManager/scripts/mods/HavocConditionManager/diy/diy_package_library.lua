...
function L.new(mod,kind,catalog,Schema,Codec,Files,Base,Packages,Hash,options)
    ...
    function self.clear_cache()
        -- Drop derived snapshots before reading a new generation. Published
        -- data survives a failed scan; active missions own separate snapshots.
        snapshot=nil;snapshot_revision=nil
        if options.before_reload and not initializing then
            local ok,why=pcall(options.before_reload)
