...
function F.new(ffi,mod_name)
    ...
    local function write_tree(parent,id,files,valid_id,valid_path,overwrite)
        if not valid_id(id) or type(files)~="table" then return nil,"diy_package_path" end
        local final=parent.."/"..id
        local replacing=exists(final)
        if replacing then
            if not overwrite then return nil,"diy_package_exists" end
            local ok,why=safe_directory(final,false);if not ok then return nil,why end
        end
        local names,total={},0
        ...
        local backup
        if replacing then
            local why;backup,why=store.retire_package(id,valid_id)
            if not backup then cleanup();return nil,why end
        end
        if win.MoveFileExW(wide(staging),wide(final),8)==0 then
            if backup then win.MoveFileExW(wide(backup),wide(final),8) end
            cleanup();return nil,exists(final) and "diy_package_exists" or "diy_package_io"
        end
        return final
    end
    function store.write_package(id,files,valid_id,valid_path,overwrite)
        local ok,why=ensure_packages();if not ok then return nil,why end
        return write_tree(store.package_directory,id,files,valid_id,valid_path,overwrite)
    end
    ...
    function store.retire_package(id,valid_id)
        ...
        for i=1,1000 do
            ...
            if not exists(target) then
                if win.MoveFileExW(wide(source),wide(target),8)~=0 then return target end
