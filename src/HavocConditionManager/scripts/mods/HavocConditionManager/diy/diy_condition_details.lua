...
local words={
    ...
    builtin_directory={"Copy built-in directory","复制内置目录","複製內建目錄"},
    external_directory={"Copy external directory","复制外部目录","複製外部目錄"},
    builtin={"Built-in","内置","內建"},external={"External","外部","外部"},
    builtin_locked={"Built-in · always loaded","内置包 · 保持加载","內建套件 · 保持載入"},
    builtin_help={"Built-in packages load directly from the mod folder. The manager cannot unload them. Select their gameplay effects on the Conditions page.","内置包直接从模组目录加载，无需解压，管理器中不能停用或移除。是否应用效果仍由「词条配置」中的勾选决定。","內建套件直接從模組目錄載入，無需解壓，管理器中不能停用或移除。是否套用效果仍由「詞條配置」中的勾選決定。"},
    shadowed={"An older external copy with the same ID is ignored and kept on disk.","外部目录中有同 ID 的旧副本，已忽略并保留原文件。","外部目錄中有相同 ID 的舊副本，已忽略並保留原檔案。"},
    ...
    empty={"No conditions loaded. Place a package in the external directory and refresh.","尚未加载词条。可将词条包放入外部目录后刷新。","尚未載入詞條。可將詞條套件放入外部目錄後重新整理。"},
