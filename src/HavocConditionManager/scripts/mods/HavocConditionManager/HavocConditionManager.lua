...
local Catalog = mod:io_dofile("HavocConditionManager/scripts/mods/HavocConditionManager/native_condition_catalog")
...
mod.get_primary_havoc_conditions = function ()
	local serialized = base_mod:get(HavocConditions.storage_key)
	if serialized==nil then serialized=mod:get("havoc_circumstances_serialized") end
	local decoded = HavocConditions.decode(serialized)
	Catalog.migrate_difficulty(decoded, base_mod)
	...
	local encoded = HavocConditions.encode(selected)
	if base_mod:get(HavocConditions.storage_key) ~= encoded then base_mod:set(HavocConditions.storage_key, encoded) end
...
mod.template_schema=mod:io_dofile("HavocConditionManager/scripts/mods/HavocConditionManager/template_schema")
local Profile=mod:io_dofile("HavocConditionManager/scripts/mods/HavocConditionManager/intensity_profile")
local migrated=Profile.migrate(mod:get("native_configuration_v3"))
if migrated then mod:set("native_configuration_v3",migrated) end
mod.template_conditions=mod:io_dofile("HavocConditionManager/scripts/mods/HavocConditionManager/template_conditions")
mod.template_registry=mod:io_dofile("HavocConditionManager/scripts/mods/HavocConditionManager/template_registry")
mod.template_runtime=mod:io_dofile("HavocConditionManager/scripts/mods/HavocConditionManager/template_runtime")
mod:io_dofile("HavocConditionManager/scripts/mods/HavocConditionManager/diy_conditions")
...
mod:io_dofile("HavocConditionManager/scripts/mods/HavocConditionManager/native_runtime_hooks")
...
mod.close_condition_manager_view=function()
    ...
    if ui and ui:view_instance(VIEW_NAME) and not (ui.is_view_closing and ui:is_view_closing(VIEW_NAME)) then
...
local function state_changed(initial_call)
    ...
    if not mod.is_gameplay_enabled() and mod.finish_diy_conditions then mod.finish_diy_conditions() end
end
mod.update=function(dt)
    if mod.update_diy_conditions and (mod:is_enabled() or mod.is_gameplay_enabled()) then mod.update_diy_conditions(dt) end
end
