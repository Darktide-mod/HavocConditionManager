...
local cached_view = mod._condition_view_cache
if cached_view and cached_view.language == language then return cached_view.class end
...
-- The options module uses this same prepared UI catalog instead of loading
-- and extending SoloPlay's complete settings a second time on first opening.
mod._condition_ui_settings = SoloPlaySettings
...
HavocConditionManagerView._persist_havoc_circumstances = function (self)
	...
	if get_setting(HavocConditions.storage_key) ~= encoded then set_setting(HavocConditions.storage_key, encoded) end
	mod.refresh_condition_generators()
...
HavocConditionManagerView._on_back_pressed = function (self)
    if Paging.slider.cancel(self) then self._hcm_refresh=true; return end
...
HavocConditionManagerView.on_exit = function (self)
	Paging.numeric.cancel(self); Paging.slider.cancel(self); Paging.help.hide(self)
	HavocConditionManagerView.super.on_exit(self)
	self:_destroy_renderer()
end

mod._condition_view_cache = { language = language, class = HavocConditionManagerView }
