...
PACKAGE_LOG_ADAPTER='log=function(id,message)mod:info("[DIY %s] %s",id,message)end'
PACKAGE_FACTORY='HavocConditionManager/scripts/mods/HavocConditionManager/diy_conditions.lua'

def without_package_log_adapter(name,text):
    # Public ctx:log is an author-invoked API. It is silent unless a selected
    # package calls it; it is not retired automatic performance diagnostics.
    if name==PACKAGE_FACTORY:
        assert text.count(PACKAGE_LOG_ADAPTER)<=1, 'Duplicate package log adapter'
        return text.replace(PACKAGE_LOG_ADAPTER,'')
    return text
def strip_lua(text):
    text=text.replace('\r\n','\n')
...
def payloads(files,strip=False):
    ...
    for name,data in files.items():
        ...
        if path.suffix=='.lua' and strip:
            ...
            assert not re.search(r'mod\s*:\s*(?:info|debug|dump)\s*\(',without_package_log_adapter(name,text)), (name,'runtime diagnostic output')
            for token in ('debug_evidence','report_coordinated_block','doors_usable','route_links','door_rechecks'):
