-- Experimental native-controller build. Installed animation resources are
-- mandatory. This first integration is limited to a local session with bots.
...
local function install()
    ...
    driver={units=weak(),cache=weak(),unsupported=weak()}
...
local function activate(ctx)
    assert(local_session(),"原生近战 +20% 试验版目前仅支持本地单人/机器人房间。")
