...
check=PROJECT/'build/checks/native-melee-controller/verify_experiment.py'
previous=sys.argv
...
L=env['L']
runtime=PROJECT/'src/HavocConditionManager/scripts/mods/HavocConditionManager/diy'
...
L.globals().embedded=L.execute((runtime/'diy_template_packages.lua').read_text(encoding='utf-8'))
...
for path,source in pairs(files)do assert(embedded['starter-conditions-frenzied_assault'][path]==source,path)end
local count=0;for _ in pairs(embedded)do count=count+1 end
assert(count==2 and embedded['starter-conditions-no_healing'])
...
''')
print('Formal Frenzied assault: embedded/external package parity, real script activation/cleanup, +20% native timing and no DIY unit update dependency: PASS')
