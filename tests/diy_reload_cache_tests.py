...
local scans=0
local library={scan=function()scans=scans+1;return true end,snapshot=function()return {options={enabled=true},document={entries={{}}}}end,runtime_error=function()end}
local modules={
    diy_schema={},diy_codec={},diy_catalog={},diy_library={},diy_files={},diy_bundled={read=function()return {}end},diy_network={new=function()return {finish=function()event('network_finish')end,update=function()end}end},
...
assert(mod.diy_api.minor==2 and #reload_options.bundled()==1)
assert(not mod.diy_api.register_package_source('../escape') and scans==0)
assert(mod.diy_api.register_package_source('HavocConditionPacks') and scans==1)
assert(mod.diy_api.register_package_source('HavocConditionPacks') and scans==1)
assert(#reload_options.bundled()==2 and reload_options.bundled()[2].name=='HavocConditionPacks')
assert(mod.diy_api.unregister_package_source('HavocConditionPacks') and scans==2)
assert(mod.diy_api.unregister_package_source('HavocConditionPacks') and scans==2 and #reload_options.bundled()==1)
