...
local MinionAnimation=require("scripts/extension_systems/animation/minion_animation_extension")
...
local function install_hooks()
    ...
    assert(AnimationSync.available(),"Frenzied assault needs the native time/animation/state restore APIs")
    ...
    hook(AiBrain,"update",function(fn,self,unit,...)
        ...
        if not nodes then
            ...
            local function visit(node)
                ...
                if node.tree_node and node.tree_node.action_data then
                    ...
                    if kind~="native" then
                        nodes[#nodes+1]={tree_node=node.tree_node,native_melee=getmetatable(node)==Melee,kind=kind,name=name,cache=weak()}
                    end
                end
        ...
        for i,entry in ipairs(nodes)do
            ...
            local rate=record.node and record.node.tree_node==node and record.attack_rate
                or current:attack_rate(unit,entry.name)
    ...
    -- Record native movement-variable writes, including gas/charge changes.
    -- Do not infer a baseline by dividing a value we may have written earlier.
    for _,method in ipairs({"set_variable","anim_event_with_variable_float"})do
        local index=method=="set_variable" and 1 or 2
        hook(MinionAnimation,method,function(fn,self,...)
            local args=pack(...)
            local current=controller(state,self._unit)
            local record=current and current.moves[self._unit]
            if record and not record.setting_move_variable and args[index]=="anim_move_speed" then
                record.move_variable_base=args[index+1]
            end
            return fn(self,unpack(args,1,args.n))
        end)
    end
    hook(MinionMovement,"apply_animation_wanted_movement_speed",function(fn,unit,navigation,dt)
        local current=controller(state,unit)
        local record=current and current.moves[unit]
        if record and record.nav==navigation and record.modifier
            and node_kind(record.brain and record.brain._running_leaf_node)=="move" then
            -- Root-derived speed already includes native playback speed.
            -- Cancel only our navigation multiplier on this feedback path.
            local speed=MinionMovement.get_animation_wanted_movement_speed(unit,dt)
            navigation:set_max_speed(speed/MOVE)
            return speed
        end
        return fn(unit,navigation,dt)
    end)
    ...
    hook(ScriptWorld,"update",function(fn,world,dt,...)
        ...
        if snapshots and state.current==current and current.active then
            local updated,layers,failed=AnimationSync.after(snapshots)
            if updated>0 and not current.animation_started then
                current.animation_started=true
                current.log("[Frenzied assault "..REVISION.."] animation_clock=verified restored_units="..updated.." restored_layers="..layers.." readback=live_clock phase=before_animation")
            end
            if failed>0 and not current.animation_failed then
                current.animation_failed=true
                current.log("[Frenzied assault "..REVISION.."] animation_clock=restore_failed layers="..failed.." readback=live_clock")
            end
        end
        return unpack(results,1,results.n)
    end)
...
local function activate(ctx)
    ...
    function current:animation_rate(brain)
        ...
        if kind~="attack" then return 1 end
    ...
    function current:sync_motion(record)
        local brain,animation=record.brain,record.animation
        ...
        -- State restoration skips root motion; its attack path retains the
        ...
        if not record.move_variable_index or not live_extension(brain._unit,"animation_system",animation) then return end
        local moving=kind=="move" and not traversal and not reacting
            and not (pad.behavior_component and pad.behavior_component.move_state=="idle")
        local value=record.move_variable_base*(moving and MOVE or 1)
        local bounds=animation._animation_variable_bounds
        bounds=bounds and bounds.anim_move_speed
        if bounds then value=math.clamp(value,bounds[1],bounds[2])end
        if Unit.animation_get_variable(brain._unit,record.move_variable_index)~=value then
            record.setting_move_variable=true
            local ok,why=pcall(animation.set_variable,animation,"anim_move_speed",value)
            record.setting_move_variable=nil
            if not ok then error(why,0)end
        end
    end
    function current:movement(unit,breed,brain)
        ...
        local record={nav=nav,loco=loco,multiplier=1,brain=brain,animation=animation};self.moves[unit]=record
        if live_extension(unit,"navigation_system",nav) then record.modifier=nav:add_movement_modifier(MOVE)end
        if animation and animation.has_variable then
            record.move_variable_index=animation:has_variable("anim_move_speed")
            if record.move_variable_index then record.move_variable_base=Unit.animation_get_variable(unit,record.move_variable_index)end
        end
    ...
    ctx:on_cleanup(function()
        ...
        for unit,record in pairs(current.moves)do
            ...
            if record.move_variable_index and live_extension(unit,"animation_system",record.animation) then
                record.animation:set_variable("anim_move_speed",record.move_variable_base)
            end
        end
    ...
    ctx:log(string.format("[Frenzied assault %s] move=%.2f melee=+%.2f ranged=+%.2f special=%.2f stagger=native movement_animation=native_variable traversal=native attack_animation=layer_restore scope=local_authority",REVISION,MOVE,ATTACK-1,RANGED-1,ATTACK))
end
