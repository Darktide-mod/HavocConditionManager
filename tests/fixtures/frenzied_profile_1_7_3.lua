...
function P.new(log,world,clock)
    ...
    function p:reset()
        ...
        self.memory_queries,self.memory_queries_max,self.memory_regions,self.memory_regions_max=0,0,0,0
    ...
    function p:report(reason)
        ...
        self.log(string.format("[Frenzied perf] version=1.7.3 reason=%s interval_frames=%d fps=%.1f frame_ms=%.2f p95_ms=%.2f max_ms=%.2f samples=%d tracked=%.1f/%d animated=%.1f/%d prepare_ms=%.3f prepare_max_ms=%.3f world_ms=%.3f world_max_ms=%.3f after_ms=%.3f ai_calls=%d ai_own_total_ms=%.3f ai_inner_total_ms=%.3f node_swaps=%d memory_queries=%.1f/%d memory_regions=%.1f/%d",
            ...
            self.memory_queries/samples,self.memory_queries_max,self.memory_regions/samples,self.memory_regions_max))
    end
    ...
    function p:ended(a,b,c,d,tracked,animated,memory)
        ...
        local queries,regions=memory and memory.queries or 0,memory and memory.regions or 0
        self.memory_queries=self.memory_queries+queries;self.memory_regions=self.memory_regions+regions
        self.memory_queries_max=math.max(self.memory_queries_max,queries);self.memory_regions_max=math.max(self.memory_regions_max,regions)
