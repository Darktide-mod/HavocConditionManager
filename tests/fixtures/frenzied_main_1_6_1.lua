...
local move_nodes=names([[BtMeleeFollowTargetAction BtRangedFollowTargetAction BtMoveToCombatVectorAction
    ...
    BtDashAction BtExitSpawnerAction BtClimbAction BtJumpAcrossAction]])
...
local move_timings=names([[align_durations align_rotation_durations anim_driven_anim_event_durations
    anim_timings blend_timings dash_direction_durations dash_durations durations end_duration end_durations
    jump_durations land_timings lunge_duration move_during_lunge_duration reached_destination_durations
    rotation_duration rotation_durations start_move_event_anim_speed_durations start_move_rotation_timings
    start_rotation_durations move_start_timings start_rotation_timings fuse_timer]])
...
local function factor(key,native_melee,kind,name,rate)
    ...
    if kind=="move" then return move_timings[key] and 1/MOVE or nil end
...
local function install_hooks()
    local state=mod._diy_frenzied_assault_hooks_v1
    if state then
        if state.revision~=REVISION then
            if state.warned_revision~=REVISION then
                state.warned_revision=REVISION
                mod:notify(mod:localize("frenzied_restart_required",tostring(state.revision),REVISION))
            end
            error(string.format("Frenzied assault hooks=%s package=%s: restart the game before starting another mission",tostring(state.revision),REVISION),0)
        end
        return state
    end
    assert(AnimationSync.available(),"Frenzied assault needs the native time/animation/state restore APIs")
    state={revision=REVISION};mod._diy_frenzied_assault_hooks_v1=state
    mod:hook(AiBrain,"update",function(fn,self,unit,...)
    ...
    mod:hook(Melee,"_start_attack_anim",function(fn,self,unit,breed,target,t,spawn,pad,data,...)
    ...
    for _,spec in ipairs({{HoundLeap,3},{Leap,2}})do
        ...
        mod:hook(class,"_check_leap_for_collisions",function(fn,self,pad,...)
    ...
    for _,method in ipairs({"set_wanted_velocity","set_wanted_velocity_flat"})do
        mod:hook(Locomotion,method,function(fn,self,velocity)
    ...
    mod:hook(MinionAttack,"start_shooting",function(fn,unit,pad,t,data,...)
    ...
    mod:hook(ScriptWorld,"update",function(fn,world,dt,...)
        local current=state.current
    ...
    mod:hook(Locomotion,"set_anim_translation_scale",function(fn,self,value)
        local current=controller(state,self._unit)
        local record=current and current.moves[self._unit]
        if record and record.loco==self then
            ...
            return fn(self,value*record.multiplier)
        end
        return fn(self,value)
    end)
    mod:hook(Locomotion,"anim_translation_scale",function(fn,self)
        local current=controller(state,self._unit)
        local record=current and current.moves[self._unit]
...
local function activate(ctx)
    ...
    function current:movement(unit,breed,brain)
        ...
        local multiplier=MOVE
        local record={nav=nav,loco=loco,multiplier=multiplier,brain=brain};self.moves[unit]=record
        if live_extension(unit,"navigation_system",nav) then record.modifier=nav:add_movement_modifier(multiplier)end
    ...
    ctx:log(string.format("[Frenzied assault %s] move=%.2f melee=+%.2f ranged=+%.2f special=%.2f stagger=native animation=layer_restore scope=local_authority",REVISION,MOVE,ATTACK-1,RANGED-1,ATTACK))
