...
local function capture(snapshot,unit,rate,dt)
    snapshot=snapshot or {times={},states={},animations={},next_times={},next_states={},next_animations={},
        native_rates={},targets={},restore_states={},restore_animations={},readback={}}
...
local function restore(snapshot)
    ...
    for i=1,snapshot.count do
        ...
        if before and native_rate and native_rate>0 and snapshot.states[i]~=nil and snapshot.animations[i]~=nil
    ...
    for i=1,count do
        if targets[i] then
            ...
            if not actual or math.abs(actual-targets[i])>.0001 then snapshot.failed=snapshot.failed+1 end
        end
    end
end
